...
function fetchAndUpdateContent(url) {
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            // Сначала определяем, какие серверы новые, чтобы добавить их
            const newServers = new Map(Object.entries(data));
            const serversToRemove = [];

            // Проверяем, какие серверы удалены
            ...
            // Добавляем или обновляем карточки серверов
